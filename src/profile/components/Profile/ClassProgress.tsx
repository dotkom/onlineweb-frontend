...
export const Circle = ({ x, radius, lineWidth, color = '#333333' }: ICircleProps) => (
  <circle cx={x} cy="10" r={radius - lineWidth / 2} strokeWidth={lineWidth} stroke={color} fill="#ffffff" />
);

export const CircleCheck = ({ x, radius, color = '#0060a3' }: ICircleCheckProps) => (
  <circle cx={x} cy="10" r={radius} stroke="none" fill={color} />
);
...
export const DividerBar = ({ offset = 0, radius, scale = 1, lineWidth, a = 0, b = 0 }: IDividerBarProps) => (
  <rect x={offset + radius * a * 2 + b * scale - lineWidth / 2} y={10 - radius} width={lineWidth} height={radius * 2} />
);
